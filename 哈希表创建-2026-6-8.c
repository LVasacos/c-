...
/*bucket是一个指向桶的桶数组，指向数组的第一个桶，每个桶是一个指向链表第一个元素的指针；
第1层: bucket (HashEntry**)
        ↓
第2层: bucket[0], bucket[1], ..., bucket[size-1]  ← 这些都是 HashEntry*（桶）
        ↓           ↓                    ↓
第3层: 链表结点    链表结点            链表结点
...
HashTable* create_hash(int size){
    ...
    hash_table->bucket=(HashEntry**)malloc(size*sizeof(HashEntry*));//申请size个指针的存储空间
    if(hash_table->bucket==NULL){free(hash_table); return NULL;}
